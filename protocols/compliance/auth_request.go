...
// to sender server in order to obtain blocksafe.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	senderBlocksafeToml, err := blocksafetoml.GetBlocksafeTomlByAddress(sender)
	if err != nil {
		return errors.Wrap(err, "Cannot get blocksafe.toml of sender domain")
	}

	if senderBlocksafeToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in blocksafe.toml of sender")
	}

	kp, err := keypair.Parse(senderBlocksafeToml.SigningKey)
	if err != nil {
