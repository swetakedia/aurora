...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartekblocksafe.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartekblocksafe.org does not validate as blocksafe_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
