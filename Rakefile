...
namespace :xdr do
  ...
  # As blocksafe-core adds more .x files, we'll need to update this array
  # Prior to launch, we should be separating our .x files into a separate
  # repo, and should be able to improve this integration.
  HAYASHI_XDR = [
                 "src/xdr/Blocksafe-types.x",
                 "src/xdr/Blocksafe-ledger-entries.x",
                 "src/xdr/Blocksafe-transaction.x",
                 "src/xdr/Blocksafe-ledger.x",
                 "src/xdr/Blocksafe-overlay.x",
                 "src/xdr/Blocksafe-SCP.x",
                ]
  ...
  task :download do
    ...
    HAYASHI_XDR.each do |src|
      ...
      encoded    = client.contents("blocksafe/blocksafe-core", path: src).content
