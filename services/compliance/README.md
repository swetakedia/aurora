...
The `compliance.cfg` file must be present in a working directory (you can load another file by using `-c` parameter). Here is an [example configuration file](https://github.com/blocksafe/bridge-server/blob/master/compliance_example.cfg). Config file should contain following values:
...
* `keys`
  * `signing_seed` - The secret seed that will be used to sign messages. Public key derived from this secret key should be in your `blocksafe.toml` file.
...
Process auth request from external organization sent before sending a payment. Check [Compliance protocol](https://www.blocksafe.org/developers/learn/integration-guides/compliance-protocol.html) for more info. It also saves memo preimage to the database.
...
Read more in [Compliance protocol](https://www.blocksafe.org/developers/learn/integration-guides/compliance-protocol.html#auth_server) doc.

#### Response

Returns [Auth response](https://www.blocksafe.org/developers/learn/integration-guides/compliance-protocol.html#reply).
...
`sender` | required | Blocksafe address (ex. `bob*blocksafe.org`) of payment sender account.
`destination` | required | Account ID or Blocksafe address (ex. `bob*blocksafe.org`) of payment destination account
...
name | description
--- | ---
`address` | Blocksafe address (ex. `alice*acme.com`) of the user.
...
```
### `callbacks.tx_status`
This callback should return the status of a transaction as explained in [`SEP-0004`](https://github.com/blocksafe/blocksafe-protocol/blob/master/ecosystem/sep-0004.md).

#### Request

name | description
--- | ---
`id` | Blocksafe transaction ID.
