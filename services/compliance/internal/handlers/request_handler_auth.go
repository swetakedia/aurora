...
import (
	...
	baseAmount "github.com/blocksafe/go/amount"
	"github.com/blocksafe/go/protocols/compliance"
	...
	httpHelpers "github.com/blocksafe/go/services/internal/bridge-compliance-shared/http/helpers"
	callback "github.com/blocksafe/go/services/internal/bridge-compliance-shared/protocols/compliance"
...
func (rh *RequestHandler) HandlerAuth(w http.ResponseWriter, r *http.Request) {
	...
	senderBlocksafeToml, err := rh.BlocksafeTomlResolver.GetBlocksafeTomlByAddress(authData.Sender)
	if err != nil {
		log.WithFields(log.Fields{"err": err, "sender": authData.Sender}).Warn("Cannot get blocksafe.toml of sender")
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "Cannot get blocksafe.toml of sender")
		httpHelpers.Write(w, errorResponse)
		return
	}

	if !shared.IsValidAccountID(senderBlocksafeToml.SigningKey) {
		errorResponse := httpHelpers.NewInvalidParameterError("data.sender", "SIGNING_KEY in blocksafe.toml of sender is invalid")
		// TODO
		// log.WithFields(errorResponse.LogData).Warn("SIGNING_KEY in blocksafe.toml of sender is invalid")
		httpHelpers.Write(w, errorResponse)
		return
	}
	...
	err = rh.SignatureSignerVerifier.Verify(senderBlocksafeToml.SigningKey, []byte(authreq.DataJSON), signatureBytes)
	if err != nil {
		log.WithFields(log.Fields{
			"signing_key": senderBlocksafeToml.SigningKey,
			"data":        authreq.Data,
	...
	if authData.NeedInfo {
		if rh.Config.Callbacks.AskUser == "" {
			...
			if len(tokens) != 2 {
				...
				}).Warn("Invalid blocksafe address")
				httpHelpers.Write(w, httpHelpers.InternalServerError)
				return
			}
