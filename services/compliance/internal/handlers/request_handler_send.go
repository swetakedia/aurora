...
import (
	...
	"github.com/blocksafe/go/protocols/federation"
...
func (rh *RequestHandler) HandlerSend(w http.ResponseWriter, r *http.Request) {
	...
	if authDataEntity != nil {
		var blocksafeToml *blocksafetoml.Response
		blocksafeToml, err = rh.BlocksafeTomlResolver.GetBlocksafeToml(authDataEntity.Domain)
		if err != nil {
		...
		if blocksafeToml.AuthServer == "" {
			log.Print("No AUTH_SERVER in blocksafe.toml")
			helpers.Write(w, callback.AuthServerNotDefined)
			return
		}

		rh.sendAuthData(w, blocksafeToml.AuthServer, []byte(authDataEntity.AuthData))
		return
	}
	...
	blocksafeToml, err := rh.BlocksafeTomlResolver.GetBlocksafeToml(domain)
	if err != nil {
		log.WithFields(log.Fields{
			"destination": request.Destination,
			"err":         err,
	...
	rh.sendAuthData(w, blocksafeToml.AuthServer, data)
}
