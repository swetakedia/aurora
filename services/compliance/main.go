...
import (
	...
	"github.com/blocksafe/go/services/compliance/internal/handlers"
	supportConfig "github.com/blocksafe/go/support/config"
	"github.com/blocksafe/go/support/db/schema"
	"github.com/blocksafe/go/support/errors"
	supportHttp "github.com/blocksafe/go/support/http"
)
...
func init() {
	rootCmd = &cobra.Command{
		Use:   "compliance",
		Short: "blocksafe compliance server",
		Long:  `blocksafe compliance server`,
		Run:   run,
	}
...
func NewApp(config config.Config, migrateFlag bool, versionFlag bool, version string) (app *App, err error) {
	...
	blocksafetomlClient := blocksafetoml.Client{
		HTTP: &httpClientWithTimeout,
	}

	federationClient := federation.Client{
		HTTP:        &httpClientWithTimeout,
		BlocksafeTOML: &blocksafetomlClient,
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: &blocksafetomlClient},
