...
type TransactionSubmitterInterface interface {
	SubmitTransaction(paymentID *string, seed string, operation, memo interface{}) (response aurora.TransactionSuccess, err error)
	SignAndSubmitRawTransaction(paymentID *string, seed string, tx *xdr.Transaction) (response aurora.TransactionSuccess, err error)
}

// TransactionSubmitter submits transactions to Blocksafe Network
...
// LoadAccount loads current state of Blocksafe account and creates a map entry if it didn't exist
func (ts *TransactionSubmitter) LoadAccount(seed string) (*Account, error) {
	...
	accountResponse, err := ts.Aurora.LoadAccount(ts.Accounts[seed].Keypair.Address())
	if err != nil {
		return nil, err
	}
...
func (ts *TransactionSubmitter) SignAndSubmitRawTransaction(paymentID *string, seed string, tx *xdr.Transaction) (response aurora.TransactionSuccess, err error) {
	...
	var herr *aurora.Error
	response, err = ts.Aurora.SubmitTransaction(txeB64)
	if err == nil {
		...
		herr, isAuroraError = err.(*aurora.Error)
		if !isAuroraError {
	...
	if herr != nil {
		...
		var accountResponse aurora.Account
		accountResponse, err = ts.Aurora.LoadAccount(account.Keypair.Address())
		if err != nil {
...
// SubmitTransaction builds and submits transaction to Blocksafe network
func (ts *TransactionSubmitter) SubmitTransaction(paymentID *string, seed string, operation, memo interface{}) (aurora.TransactionSuccess, error) {
	account, err := ts.LoadAccount(seed)
	if err != nil {
		return aurora.TransactionSuccess{}, errors.Wrap(err, "Error loading an account")
	}
	...
	if !ok {
		...
		return aurora.TransactionSuccess{}, errors.New("Cannot cast operationMutator to build.TransactionMutator")
	}
	...
	if memo != nil {
		...
		if !ok {
			...
			return aurora.TransactionSuccess{}, errors.New("Cannot cast memo to build.TransactionMutator")
		}
	...
	if err != nil {
		return aurora.TransactionSuccess{}, errors.Wrap(err, "Error building a transaction")
	}
