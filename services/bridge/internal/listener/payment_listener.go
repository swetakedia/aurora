...
func (pl *PaymentListener) Listen() (err error) {
	...
	_, err = pl.aurora.LoadAccount(accountID)
	if err != nil {
		return
	}
	...
	go func() {
		for {
			...
			var cursor aurora.Cursor
			if cursorValue != nil {
				cursor = aurora.Cursor(*cursorValue)
			} else {
			...
			err = pl.aurora.StreamPayments(context.Background(), accountID, &cursor, pl.onPayment)
			if err != nil {
...
func (pl *PaymentListener) ReprocessPayment(payment aurora.Payment, force bool) error {
...
func (pl *PaymentListener) onPayment(payment aurora.Payment) {
...
func (pl *PaymentListener) shouldProcessPayment(payment aurora.Payment) (bool, string) {
...
func (pl *PaymentListener) process(payment aurora.Payment) error {
	if payment.Type == "account_merge" {
		payment.AssetType = "native"
		payment.From = payment.Account
		payment.To = payment.Into

		err := pl.aurora.LoadAccountMergeAmount(&payment)
		if err != nil {
			return errors.Wrap(err, "Unable to load account_merge amount")
		}
	}

	err := pl.aurora.LoadMemo(&payment)
	if err != nil {
