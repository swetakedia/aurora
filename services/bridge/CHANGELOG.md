...
* Fix for a bug in `/builder` endpoint: sequence number is now incremented when loaded from Aurora server (https://github.com/blocksafe/bridge-server/issues/86).
...
* Added error messages to Compliance protocol ([SEP-0003](https://github.com/blocksafe/blocksafe-protocol/blob/master/ecosystem/sep-0003.md))

## 0.0.27

* Admin Panel (`/admin` endpoint in `bridge` server).
* `/tx_status` endpoint [More info](https://github.com/blocksafe/blocksafe-protocol/blob/master/ecosystem/sep-0001.md).
...
* [XLM (lumen)](https://www.blocksafe.org/lumens/) payments can be now used in `PaymentListener`.
...
* Update `github.com/blocksafe/go` dependency.
...
* Change blocksafe.toml location to new standard.
...
* [Compliance protocol](https://www.blocksafe.org/developers/learn/integration-guides/compliance-protocol.html) support.
...
* Sending to Blocksafe address with memo in `/send`.
...
* Fixed sending to Blocksafe address.
