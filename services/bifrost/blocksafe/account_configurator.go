...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("BlocksafeAccountConfigurator")
	ac.log.Info("BlocksafeAccountConfigurator starting")
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring Blocksafe account")
	...
	for {
		...
		localLog.WithField("destination", destination).Info("Creating Blocksafe account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating Blocksafe account")
			time.Sleep(2 * time.Second)
			continue
		}
