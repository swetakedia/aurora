...
func (m *MockDatabase) CreateAddressAssociation(chain Chain, blocksafeAddress, address string, addressIndex uint32) error {
	a := m.Called(chain, blocksafeAddress, address, addressIndex)
	return a.Error(0)
}
...
func (m *MockDatabase) GetAssociationByBlocksafePublicKey(blocksafePublicKey string) (*AddressAssociation, error) {
	a := m.Called(blocksafePublicKey)
	return a.Get(0).(*AddressAssociation), a.Error(1)
}
