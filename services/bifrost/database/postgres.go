...
func fromQueueTransaction(tx queue.Transaction) *transactionsQueueRow {
	return &transactionsQueueRow{
		...
		Amount:           tx.Amount,
		BlocksafePublicKey: tx.BlocksafePublicKey,
	}
}
...
func (r *transactionsQueueRow) toQueueTransaction() *queue.Transaction {
	return &queue.Transaction{
		...
		Amount:           r.Amount,
		BlocksafePublicKey: r.BlocksafePublicKey,
	}
}
...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, blocksafeAddress, address string, addressIndex uint32) error {
	...
	association := &AddressAssociation{
		...
		Address:          address,
		BlocksafePublicKey: blocksafeAddress,
		CreatedAt:        time.Now(),
	}
...
func (d *PostgresDatabase) GetAssociationByBlocksafePublicKey(blocksafePublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"blocksafe_public_key": blocksafePublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
