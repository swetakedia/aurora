...
type Database interface {
	// CreateAddressAssociation creates Bitcoin/Ethereum-Blocksafe association. `addressIndex`
	// is the chain (Bitcoin/Ethereum) address derivation index (BIP-32).
	CreateAddressAssociation(chain Chain, blocksafeAddress, address string, addressIndex uint32) error
	// GetAssociationByChainAddress searches for previously saved Bitcoin/Ethereum-Blocksafe association.
	// Should return nil if not found.
	GetAssociationByChainAddress(chain Chain, address string) (*AddressAssociation, error)
	// GetAssociationByBlocksafePublicKey searches for previously saved Bitcoin/Ethereum-Blocksafe association.
	// Should return nil if not found.
	GetAssociationByBlocksafePublicKey(blocksafePublicKey string) (*AddressAssociation, error)
...
type AddressAssociation struct {
	...
	BlocksafePublicKey string    `db:"blocksafe_public_key"`
