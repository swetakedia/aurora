...
func (suite *BitcoinRailTestSuite) TestAssociationAlreadyProcessed() {
	...
	association := &database.AddressAssociation{
		...
		BlocksafePublicKey: "GDULKYRRVOMASFMXBYD4BYFRSHAKQDREEVVP2TMH2CER3DW2KATIOASB",
		CreatedAt:        time.Now(),
	}
	suite.MockDatabase.
...
func (suite *BitcoinRailTestSuite) TestAssociationSuccess() {
	...
	suite.MockQueue.
		...
		Run(func(args mock.Arguments) {
			...
			suite.Assert().Equal(association.BlocksafePublicKey, queueTransaction.BlocksafePublicKey)
		})
	suite.MockSSEServer.
