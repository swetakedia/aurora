...
// the transactions queue for BlocksafeAccountConfigurator to consume.
//
...
func (s *Server) onNewBitcoinTransaction(transaction bitcoin.Transaction) error {
	...
	queueTx := queue.Transaction{
		...
		Amount:           transaction.ValueToBlocksafe(),
		BlocksafePublicKey: addressAssociation.BlocksafePublicKey,
	}
