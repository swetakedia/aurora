...
func (s *Server) Start() error {
	...
	s.BlocksafeAccountConfigurator.OnAccountCreated = s.onBlocksafeAccountCreated
	s.BlocksafeAccountConfigurator.OnExchanged = s.onExchanged
	s.BlocksafeAccountConfigurator.OnExchangedTimelocked = s.OnExchangedTimelocked
	...
	err := s.BlocksafeAccountConfigurator.Start()
	if err != nil {
		return errors.Wrap(err, "Error starting BlocksafeAccountConfigurator")
	}
...
func (s *Server) handlerGenerateAddress(w stdhttp.ResponseWriter, r *stdhttp.Request, chain database.Chain) {
	...
	blocksafePublicKey := r.PostFormValue("blocksafe_public_key")
	_, err := keypair.Parse(blocksafePublicKey)
	if err != nil || (err == nil && blocksafePublicKey[0] != 'G') {
		log.WithField("blocksafePublicKey", blocksafePublicKey).Warn("Invalid blocksafePublicKey")
		w.WriteHeader(stdhttp.StatusBadRequest)
		return
	}
	...
	err = s.Database.CreateAddressAssociation(chain, blocksafePublicKey, address, index)
	if err != nil {
		log.WithFields(log.F{
			"err":              err,
			"chain":            chain,
			"index":            index,
			"blocksafePublicKey": blocksafePublicKey,
			"address":          address,
