...
func (s *Server) onBlocksafeAccountCreated(destination string) {
	association, err := s.Database.GetAssociationByBlocksafePublicKey(destination)
	if err != nil {
	...
	if association == nil {
		s.log.WithField("blocksafePublicKey", destination).Error("Association not found")
		return
	}
