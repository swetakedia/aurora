...
Bifrost is highly available and secure Bitcoin/Ethereum → Blocksafe bridge. It allows users to move BTC/ETH to Blocksafe network and then trade them to other tokens or participate in ICOs (initial coin offering).

It solves many problems connected to moving tokens to Blocksafe network:

* Security:
  * Developers don’t have access to users’ Blocksafe keys.
...
Download the binary from [the release page](https://github.com/blocksafe/go/releases/tag/bifrost-v0.0.2) and use it with it's [Bifrost JS SDK](https://github.com/blocksafe/bifrost-js-sdk).

## How it works

1. User opens the web app implemented using [Bifrost JS SDK](https://github.com/blocksafe/bifrost-js-sdk).
1. User is presented with her public and private Blocksafe keys where Bitcoin/Ethereum will be sent.
1. User selects what cryptocurrency she wants to move to Blocksafe network.
1. A receiving Bitcoin/Ethereum address is generated.
1. User sends funds in Bitcoin/Ethereum network.
1. Bifrost listens to Bitcoin and Ethereum network events. When payment arrives it creates a Blocksafe [account](https://www.blocksafe.org/developers/guides/concepts/accounts.html) for the user who later adds a temporary signer on the account.
1. Using a temporary signer Bifrost creates necessary [trust lines](https://www.blocksafe.org/developers/guides/concepts/assets.html) and sends corresponding amount of BTC/ETH to user's Blocksafe account. It also exchanges BTC/ETH to the final token at a given rate.
...
1. Now open Bifrost demo: https://bifrost.blocksafe.org/ It will display an Ethereum address for sending your test ETH.
...
* `bitcoin`
  ...
  * `token_price` - a price of one BTC in terms of final token (`blocksafe.token_asset_code`)
* `ethereum`
  ...
  * `token_price` - a price of one ETH in terms of final token (`blocksafe.token_asset_code`)
* `blocksafe`
  ...
  * `signer_secret_key` - distribuions accounts's secret key if only one instance of Bifrost is deployed OR [channel](https://www.blocksafe.org/developers/guides/channels.html)'s secret key if more than one instance of Bifrost is deployed, signer is also used as a temporary signer in new accounts (see "Account configuration" section)
  * `aurora` - URL to [aurora](https://github.com/blocksafe/go/tree/master/services/aurora) server
  * `network_passphrase` - Blocksafe network passphrase (`Public Global Blocksafe Network ; September 2015` for production network, `Test SDF Network ; September 2015` for test network)
  * `starting_balance` - Blocksafe XLM amount issued to created account (41 by default)
...
This is the most basic configuration but it is not recommended. In this configuration only one account is responsible for everything: issuing and distributing assets and creating new accounts. In this configuration `blocksafe.issuer_public_key`, `blocksafe.distribution_public_key` and `blocksafe.signer_secret_key` represent a single account.

### Issuing and Distribution Accounts

Sometimes you want to have a limited supply of your token so you create a distribution account, send all the tokens to distribution (and other accounts, ex. vesting) and lock the issuing account so no new tokens can be created. In this configuration `blocksafe.distribution_public_key` and `blocksafe.signer_secret_key` represent a separate account used to distribute funds in a token sale. This account will also be used to create new accounts.

### Distribution Account with multiple signers

If you expect really high load you should deploy multiple Bifrost servers. To prevent Bifrost instances from consuming the same sequence numbers of the Distribution account you should use [channels](https://www.blocksafe.org/developers/guides/channels.html). Each Bifrost server will contain a secret key of one of the signers of Distribution account and will use Signer account as a [transaction source](https://www.blocksafe.org/developers/guides/concepts/transactions.html#source-account) when creating new accounts.

In this configuration all servers have the same `blocksafe.issuer_public_key` and `blocksafe.distribution_public_key` values but each server has a separate `blocksafe.signer_secret_key` which is a signer of `blocksafe.distribution_public_key` account (and needs to exist in Blocksafe ledger).
...
* Make sure "Sell [your token] for BTC" and/or "Sell [your token] for ETH" exist in Blocksafe production network. If not, you can create an offer by sending a transaction with `manage_offer` operation.
* Make sure you don't use account from `blocksafe.issuer_secret_key` anywhere else than bifrost. Otherwise, sequence numbers will go out of sync and bifrost will stop working. It's good idea to create a new signer on issuing account.
...
* Deploy at least 2 bifrost, bitcoin-core, geth, blocksafe-core and aurora servers. Use multi-AZ database.
...
* It's a good idea to set "Authorization required" and "Authorization revocable" [flags](https://www.blocksafe.org/developers/guides/concepts/accounts.html#flags) on issuing account during ICO stage to remove trustlines to accounts with lost keys. You need to set `blocksafe.needs_authorize` in config to `true` if you want to use this feature.
...
* `Bifrost.ExchangedTimelockedEvent` [event](https://github.com/blocksafe/bifrost-js-sdk#oneventevent-data). You can display it to the user if you want your users to unlock accounts.
...
Check [Blocksafe Stack Exchange](https://blocksafe.stackexchange.com/questions/tagged/bifrost/) to find similar questions or add the new question with the `bifrost` tag.
