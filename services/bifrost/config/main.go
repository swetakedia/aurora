...
type Config struct {
	...
	Blocksafe struct {
		...
		IssuerPublicKey string `valid:"required,blocksafe_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,blocksafe_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.blocksafe.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,blocksafe_seed" toml:"signer_secret_key"`
		...
		StartingBalance string `valid:"optional,blocksafe_amount" toml:"starting_balance"`
...
func (c Config) SignerPublicKey() string {
	kp := keypair.MustParse(c.Blocksafe.SignerSecretKey)
	return kp.Address()
}
