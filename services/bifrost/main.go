...
import (
	...
	"github.com/blocksafe/go/services/bifrost/server"
	"github.com/blocksafe/go/services/bifrost/sse"
	"github.com/blocksafe/go/services/bifrost/blocksafe"
	"github.com/blocksafe/go/services/bifrost/stress"
	supportConfig "github.com/blocksafe/go/support/config"
...
var rootCmd = &cobra.Command{
	Use:   "bifrost",
	Short: "Bridge server to allow participating in Blocksafe based ICOs using Bitcoin and Ethereum",
}
...
This command will create 3 server.Server's listening on ports 8000-8002.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for i := 0; i < numServers; i++ {
			go func(i int) {
				cfg.Port = ports[i]
				cfg.Blocksafe.SignerSecretKey = signers[i]
		...
		users := stress.Users{
			Aurora: &aurora.Client{
				URL: cfg.Blocksafe.Aurora,
				HTTP: &http.Client{
					Timeout: 60 * time.Second,
				},
			},
			NetworkPassphrase: cfg.Blocksafe.NetworkPassphrase,
			UsersPerSecond:    usersPerSecond,
			BifrostPorts:      ports,
			IssuerPublicKey:   cfg.Blocksafe.IssuerPublicKey,
		}
...
func init() {
	// TODO I think these should be default in blocksafe/go:
...
func createServer(cfg config.Config, stressTest bool) *server.Server {
	...
	blocksafeAccountConfigurator := &blocksafe.AccountConfigurator{
		NetworkPassphrase:     cfg.Blocksafe.NetworkPassphrase,
		IssuerPublicKey:       cfg.Blocksafe.IssuerPublicKey,
		DistributionPublicKey: cfg.Blocksafe.DistributionPublicKey,
		SignerSecretKey:       cfg.Blocksafe.SignerSecretKey,
		NeedsAuthorize:        cfg.Blocksafe.NeedsAuthorize,
		TokenAssetCode:        cfg.Blocksafe.TokenAssetCode,
		StartingBalance:       cfg.Blocksafe.StartingBalance,
		LockUnixTimestamp:     cfg.Blocksafe.LockUnixTimestamp,
	}

	if cfg.Blocksafe.StartingBalance == "" {
		blocksafeAccountConfigurator.StartingBalance = "2.1"
	}

	if cfg.Bitcoin != nil {
		blocksafeAccountConfigurator.TokenPriceBTC = cfg.Bitcoin.TokenPrice
	}

	if cfg.Ethereum != nil {
		blocksafeAccountConfigurator.TokenPriceETH = cfg.Ethereum.TokenPrice
	}
	...
	err = g.Provide(
		...
		&inject.Object{Value: blocksafeAccountConfigurator},
	)
	if err != nil {
