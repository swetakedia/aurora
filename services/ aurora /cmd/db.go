...
import (
	...
	hlog "github.com/blocksafe/go/support/log"
)
...
var dbInitAssetStatsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		cdb, err := db.Open("postgres", config.BlocksafeCoreDatabaseURL)
		if err != nil {
			log.Fatal(err)
		}
...
var dbRebaseCmd = &cobra.Command{
	Use:   "rebase",
	Short: "rebases clears the aurora db and ingests the latest ledger segment from blocksafe-core",
	Long:  "...",
	Run: func(cmd *cobra.Command, args []string) {
		hlog.DefaultLogger.Logger.Level = config.LogLevel
		initConfig()
...
func ingestSystem(ingestConfig ingest.Config) *ingest.System {
	...
	return ingest.New(passphrase, config.BlocksafeCoreURL, cdb, hdb, ingestConfig)
}
