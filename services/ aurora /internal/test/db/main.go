...
import (
	...
	_ "github.com/lib/pq"
	db "github.com/blocksafe/go/support/db/dbtest"
)
...
// BlocksafeCore returns a connection to the blocksafe core test database
func BlocksafeCore(t *testing.T) *sqlx.DB {
	if coreDB != nil {
		return coreDB
	}
	postgres := db.Postgres(t)
...
// BlocksafeCoreURL returns the database connection the url any test
// use when connecting to the blocksafe-core database
func BlocksafeCoreURL() string {
	if coreUrl == nil {
		log.Panic(fmt.Errorf("BlocksafeCore not initialized"))
	}
	return *coreUrl
}
