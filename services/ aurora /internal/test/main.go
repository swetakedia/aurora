...
import (
	...
	"github.com/blocksafe/go/services/aurora/internal/logmetrics"
	tdb "github.com/blocksafe/go/services/aurora/internal/test/db"
...
// LoadScenarioWithoutAurora populates the test Blocksafe core database a with
...
func Start(t *testing.T) *T {
	...
	result.CoreDB = BlocksafeCoreDatabase(t)
...
// BlocksafeCoreDatabase returns a connection to the blocksafe core test database
//
// DEPRECATED:  use `BlocksafeCore()` from test/db package
func BlocksafeCoreDatabase(t *testing.T) *sqlx.DB {
	return tdb.BlocksafeCore(t)
}

// BlocksafeCoreDatabaseURL returns the database connection the url any test
// use when connecting to the blocksafe-core database
//
// DEPRECATED:  use `BlocksafeCoreURL()` from test/db package
func BlocksafeCoreDatabaseURL() string {
	return tdb.BlocksafeCoreURL()
}
