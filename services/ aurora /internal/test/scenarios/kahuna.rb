...
# blocksafe's features, exercising each feature and option at least once.
#
# As new features are added to blocksafe, this scenario will get updated to
# exercise those new features.  This scenario is used during the aurora
# ingestion tests.
#
use_manual_close
KP = Blocksafe::KeyPair
close_ledger #2
...
# time bounds
  ...
  create_account :time_bounds do |env|
    env.tx.time_bounds = Blocksafe::TimeBounds.new(min_time: 100, max_time: Time.parse("2020-01-01").to_i)
...
# different source account
  ...
  payment :master, :different_source,  [:native, "10.00"] do |env|
    newop = Blocksafe::Operation.from_xdr env.tx.operations[0].to_xdr
