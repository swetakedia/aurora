...
# one-time signer being consumed
  ...
  key = Blocksafe::SignerKey.hash_x(x)
  set_options :onetime, signer: Blocksafe::Signer.new({
    key: key, 
    weight: 1,
  })
  close_ledger #4
  ...
  tx = Blocksafe::Transaction.manage_data({
    account:  account,
  ...
  env = tx.to_envelope
  env.signatures << Blocksafe::DecoratedSignature.new({
