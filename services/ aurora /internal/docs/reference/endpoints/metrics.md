...
```sh
curl "https://aurora-testnet.blocksafe.org/metrics"
```
...
Aurora maintains its own database (postgres), a verbose and user friendly account of activity on the Blocksafe network.
...
#### Ingester
Ingester represents metrics specific to Aurora's [ingestion](https://github.com/blocksafe/go/blob/master/services/aurora/internal/docs/reference/admin.md#ingesting-blocksafe-core-data) process, or the process by which Aurora consumes transaction results from a connected Blocksafe Core instance.
...
#### Blocksafe Core
As noted above, Aurora relies on Blocksafe Core to stay in sync with the Blocksafe network. These metrics are specific to the underlying Blocksafe Core instance.

|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| latest_ledger    | The sequence number of the latest (most recent) ledger recorded in Blocksafe Core's database.  |
| open_connections | The number of open connections to the Blocksafe Core postgres database.  |

##### *Example Response:*
```shell
"blocksafe_core.latest_ledger": {
  "value": 19203710
},
"blocksafe_core.open_connections": {
  "value": 4
},
```
...
Aurora does not submit transactions directly to the Blocksafe network. Instead, it sequences transactions and sends the base64 encoded, XDR serialized blob to its connected Blocksafe Core instance. 

##### Aurora Transaction Sequencing and Submission

The following is a simplified version of the transaction submission process that glosses over the finer details. To dive deeper, check out the [source code](https://github.com/blocksafe/go/tree/master/services/aurora/internal/txsub).

Aurora's sequencing mechanism consists of a [manager](https://github.com/blocksafe/go/blob/master/services/aurora/internal/txsub/sequence/manager.go) that keeps track of [submission queues](https://github.com/blocksafe/go/blob/master/services/aurora/internal/txsub/sequence/queue.go) for a set of addresses. A submission queue is a  priority queue, prioritized by minimum transaction sequence number, that holds a set of pending transactions for an account. A pending transaction is represented as an object with a sequence number and a channel. Periodically, this queue is updated, popping off finished transactions, sending down the transaction's channel a successful/failure response.
