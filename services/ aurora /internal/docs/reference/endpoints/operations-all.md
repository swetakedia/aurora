...
clientData:
  laboratoryUrl: https://www.blocksafe.org/laboratory/#explorer?resource=operations&endpoint=all
---
...
This endpoint can also be used in [streaming](../streaming.md) mode so it is possible to use it to listen as operations are processed in the Blocksafe network.
...
```sh
curl "https://aurora-testnet.blocksafe.org/operations?limit=200&order=desc"
```
