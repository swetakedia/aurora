...
using JavaScript and `EventSource`.  We will eschew using [`js-blocksafe-sdk`](https://github.com/blocksafe/js-blocksafe-sdk), the
...
$ cd follow_tutorial
$ npm install --save blocksafe-base
...
```javascript
var Keypair = require("blocksafe-base").Keypair;
...
The Blocksafe test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://friendbot.blocksafe.org/?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Blocksafe network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://aurora-testnet.blocksafe.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/blocksafe/js-blocksafe-sdk).
...
var es = new EventSource('https://aurora-testnet.blocksafe.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
```bash
$ curl "https://aurora-testnet.blocksafe.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
```js
var BlocksafeBase = require("blocksafe-base");
BlocksafeBase.Network.useTestNetwork();

var keypair = BlocksafeBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new BlocksafeBase.Account(keypair.publicKey(), "713226564141056");

var amount = "100";
var transaction = new BlocksafeBase.TransactionBuilder(account)
  .addOperation(BlocksafeBase.Operation.createAccount({
    destination: BlocksafeBase.Keypair.random().publicKey(),
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to aurora or blocksafe-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.
