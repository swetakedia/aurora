...
import (
	...
	auroraContext "github.com/blocksafe/go/services/aurora/internal/context"
	...
	"github.com/blocksafe/go/services/aurora/internal/ledger"
	"github.com/blocksafe/go/services/aurora/internal/operationfeestats"
	"github.com/blocksafe/go/services/aurora/internal/paths"
	"github.com/blocksafe/go/services/aurora/internal/reap"
	"github.com/blocksafe/go/services/aurora/internal/txsub"
	"github.com/blocksafe/go/support/app"
...
// CoreSession returns a new session that loads data from the blocksafe core
// database. The returned session is bound to `ctx`.
...
// UpdateBlocksafeCoreInfo updates the value of coreVersion,
// currentProtocolVersion, and coreSupportedProtocolVersion from the Blocksafe
// core API.
func (a *App) UpdateBlocksafeCoreInfo() {
	if a.config.BlocksafeCoreURL == "" {
		return
	}

	core := &blocksafecore.Client{
		URL: a.config.BlocksafeCoreURL,
	}

	resp, err := core.Info(context.Background())
	if err != nil {
		log.Warnf("could not load blocksafe-core info: %s", err)
		return
	}
	...
	if resp.Info.Network != a.config.NetworkPassphrase {
		log.Errorf(
			"Network passphrase of blocksafe-core (%s) does not match Aurora configuration (%s). Exiting...",
			resp.Info.Network,
...
func (a *App) Tick() {
	...
	// update ledger state, operation fee state, and blocksafe-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateOperationFeeStatsState(); wg.Done() }()
	go func() { a.UpdateBlocksafeCoreInfo(); wg.Done() }()
	wg.Wait()
