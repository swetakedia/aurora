...
import (
	...
	herr "github.com/blocksafe/go/services/aurora/internal/errors"
	"github.com/blocksafe/go/services/aurora/internal/ledger"
	"github.com/blocksafe/go/services/aurora/internal/toid"
	"github.com/blocksafe/go/support/errors"
	ilog "github.com/blocksafe/go/support/log"
)
...
// RebaseHistory re-establishes aurora's history database by clearing it, ingesting the latest ledger in blocksafe-core then backfilling as many ledgers as possible
...
// run causes the importer to check blocksafe-core to see if we can import new
// data.
func (i *System) runOnce() {
	...
	if coreLatest == 1 {
		log.Warn("ingest: waiting for blocksafe-core sync")
		return
	}
...
// aurora but is no longer present in the blocksafe-core database source.  The
// usual cause for this situation is a blocksafe-core that uses the CATCHUP_RECENT
// mode.
