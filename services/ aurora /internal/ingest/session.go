...
import (
	...
	"github.com/blocksafe/go/meta"
	"github.com/blocksafe/go/services/aurora/internal/db2/history"
	"github.com/blocksafe/go/services/aurora/internal/ingest/participants"
	"github.com/blocksafe/go/support/errors"
	ilog "github.com/blocksafe/go/support/log"
	sTime "github.com/blocksafe/go/support/time"
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  blocksafe-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// blocksafe-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured blocksafe-core server
// to report that it has finished processing the data being ingested.  This
// allows blocksafe-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	if is.BlocksafeCoreURL == "" {
		return nil
	}
	...
	core := &blocksafecore.Client{URL: is.BlocksafeCoreURL}
