...
type TxResult struct {
	maybeTransactionSuccess *aurora.TransactionSuccess
	maybeErr                error
}
...
func (bot *Bot) Pay(destAddress string) (*aurora.TransactionSuccess, error) {
...
func AsyncSubmitTransaction(bot *Bot, channel chan TxResult, signed string) {
	result, err := bot.Aurora.SubmitTransaction(signed)
	if err != nil {
		switch e := err.(type) {
		case *aurora.Error:
...
func (bot *Bot) checkHandleBadSequence(err *aurora.Error) {
...
func (bot *Bot) refreshSequence() error {
	botAccount, err := bot.Aurora.LoadAccount(bot.address())
	if err != nil {
		bot.sequence = 0
		return err
	}
