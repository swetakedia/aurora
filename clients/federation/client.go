...
// LookupByAddress performs a federated lookup following to the blocksafe
...
// "name" type is a legacy holdover from the legacy blocksafe network's federation
...
// LookupByAccountID performs a federated lookup following to the blocksafe
...
// ForwardRequest performs a federated lookup following to the blocksafe
...
func (c *Client) getFederationServer(domain string) (string, error) {
	stoml, err := c.BlocksafeTOML.GetBlocksafeToml(domain)
	if err != nil {
		return "", errors.Wrap(err, "get blocksafe.toml failed")
	}

	if stoml.FederationServer == "" {
		return "", errors.New("blocksafe.toml is missing federation server info")
	}
