...
func TestLookupByAddress(t *testing.T) {
	...
	tomlmock := &blocksafetoml.MockClient{}
	c := &Client{BlocksafeTOML: tomlmock, HTTP: hmock}

	// happy path - string integer
	tomlmock.On("GetBlocksafeToml", "blocksafe.org").Return(&blocksafetoml.Response{
		FederationServer: "https://blocksafe.org/federation",
	}, nil)
	hmock.On("GET", "https://blocksafe.org/federation").
		ReturnJSON(http.StatusOK, map[string]string{
			"blocksafe_address": "scott*blocksafe.org",
			"account_id":      "GASTNVNLHVR3NFO3QACMHCJT3JUSIV4NBXDHDO4VTPDTNN65W3B2766C",
			"memo_type":       "id",
			"memo":            "123",
		})
	resp, err := c.LookupByAddress("scott*blocksafe.org")
	...
	tomlmock.On("GetBlocksafeToml", "toobig.org").Return(&blocksafetoml.Response{
		FederationServer: "https://toobig.org/federation",
	}, nil)
	hmock.On("GET", "https://toobig.org/federation").
		ReturnJSON(http.StatusOK, map[string]string{
			"blocksafe_address": strings.Repeat("0", FederationResponseMaxSize) + "*blocksafe.org",
			"account_id":      "GASTNVNLHVR3NFO3QACMHCJT3JUSIV4NBXDHDO4VTPDTNN65W3B2766C",
			"memo_type":       "id",
			"memo":            "123",
		})
	...
	tomlmock.On("GetBlocksafeToml", "missing.org").Return(
		(*blocksafetoml.Response)(nil),
	...
	tomlmock.On("GetBlocksafeToml", "404.org").Return(&blocksafetoml.Response{
	...
	tomlmock.On("GetBlocksafeToml", "error.org").Return(&blocksafetoml.Response{
...
func TestForwardRequest(t *testing.T) {
	...
	resp, err := c.ForwardRequest("blocksafe.org", fields)
...
func Test_url(t *testing.T) {
	...
	furl := c.url("https://blocksafe.org/federation", qstr)
	assert.Equal(t, "https://blocksafe.org/federation?acct=2382376&federation_type=bank_account&swift=BOPBPHMM&type=forward", furl)

	// regression: ensure that query is properly URI encoded
	qstr = url.Values{}
	qstr.Add("type", "q")
	qstr.Add("q", "scott+receiver1@blocksafe.org*blocksafe.org")
	furl = c.url("", qstr)
	assert.Equal(t, "?q=scott%2Breceiver1%40blocksafe.org%2Ablocksafe.org&type=q", furl)
}
